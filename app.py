...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    try:
        quartas_preds_raw = _json.loads(_quartas_path.read_text(encoding="utf-8"))
        quartas_loaded    = True
    except Exception:
        quartas_loaded = False

    try:
        semis_preds_raw = _json.loads(_semis_path.read_text(encoding="utf-8"))
        semis_loaded    = True
    except Exception:
        semis_loaded = False
    ...
    # ── Quartas de Final ─────────────────────────────────────────────────────
    st.markdown("---")
    st.subheader("Quartas de Final — 4 confrontos")

    if not quartas_loaded:
        st.warning("Execute `python generate_quartas_predictions.py` para gerar as previsões das quartas.")
    else:
        st.caption(
            "Elo atualizado com **72 resultados reais** (grupos + R32 + todas as oitavas). "
            "France e Argentina líderam o ranking Elo desta Copa."
        )

        col_q_l, col_q_r = st.columns(2)
        for i, p in enumerate(quartas_preds_raw["jogos"]):
            ta    = p["mandante"]
            tb    = p["visitante"]
            score = p["placar_previsto"]
            wa_p  = int(round(p.get("prob_mandante",  0.33) * 100))
            ...
            target_col = col_q_l if i % 2 == 0 else col_q_r
            with target_col:
                st.markdown(card_html, unsafe_allow_html=True)
                if not jogado:
                    if st.button("🔍 Previsão completa", key=f"r8_{p['id']}", use_container_width=True):
                        st.session_state.update({
                            "selected_match": {
                                "home": ta, "away": tb,
                                "date": p["data"], "venue": venue,
                        ...
                        _load_and_predict(ta, tb, "Quartas de Final")
                        st.rerun()

        st.markdown("---")
        rows_quartas = []
        for p in quartas_preds_raw["jogos"]:
            rows_quartas.append({
                "Data":           p["data"] + " " + p["horario_brt"],
                "Mandante":       p["mandante"],
                "Visitante":      p["visitante"],
        ...
        st.dataframe(pd.DataFrame(rows_quartas), use_container_width=True, hide_index=True)

        st.caption(
            f"Previsões geradas em **{quartas_preds_raw.get('_gerado_em','')[:10]}** · "
            f"{quartas_preds_raw.get('_modelo','')}."
        )

    # ── Semifinais ───────────────────────────────────────────────────────────
    st.markdown("---")
    st.subheader("Semifinais — 2 confrontos")

    if not semis_loaded:
        st.warning("Execute `python generate_semis_predictions.py` para gerar as previsões das semifinais.")
    else:
        st.caption(
            "Elo atualizado com **76 resultados reais** desta Copa. "
            "France vs Spain está quase no fio da navalha (39%/21%/40%). "
            "Argentina lidera o ranking Elo com **2194 pts**."
        )

        col_sf_l, col_sf_r = st.columns(2)
        for i, p in enumerate(semis_preds_raw["jogos"]):
            ta    = p["mandante"]
            tb    = p["visitante"]
            score = p["placar_previsto"]
            wa_p  = int(round(p.get("prob_mandante",  0.33) * 100))
            ...
            card_html = f"""
            <div class="match-card" style="margin-bottom:10px;border:1px solid #f59e0b33;">
              <div style="font-size:.72rem;color:#64748b;margin-bottom:6px;">
                {p['id']} &nbsp;·&nbsp; {date_} &nbsp;·&nbsp; ⏰ {time_} BRT{encerrado_label}
              </div>
              <div style="display:flex;align-items:center;gap:6px;">
                <div style="flex:1;text-align:right;">
                  <div style="font-weight:700;font-size:1.05rem;">{ta}</div>
                  <div style="font-size:.7rem;color:#64748b;">Elo {p.get('elo_mandante','')}</div>
                </div>
                <div style="text-align:center;min-width:72px;">
                  <span class="score-badge" style="font-size:1.3rem;padding:4px 12px;">{score}</span>
                  <div style="font-size:.6rem;color:#64748b;margin-top:2px;">previsão</div>
                </div>
                <div style="flex:1;text-align:left;">
                  <div style="font-weight:700;font-size:1.05rem;">{tb}</div>
                  <div style="font-size:.7rem;color:#64748b;">Elo {p.get('elo_visitante','')}</div>
                </div>
              </div>
              {resultado_line}
            ...
            target_col = col_sf_l if i % 2 == 0 else col_sf_r
            with target_col:
                st.markdown(card_html, unsafe_allow_html=True)
                if not jogado:
                    if st.button("🔍 Previsão completa", key=f"sf_{p['id']}", use_container_width=True):
                        st.session_state.update({
                            "selected_match": {
                                "home": ta, "away": tb,
                                "date": p["data"], "venue": venue,
                        ...
                        _load_and_predict(ta, tb, "Semifinal")
                        st.rerun()

        st.markdown("---")
        rows_semis = []
        for p in semis_preds_raw["jogos"]:
            rows_semis.append({
                "Data":           p["data"] + " " + p["horario_brt"],
                "Mandante":       p["mandante"],
                "Visitante":      p["visitante"],
        ...
        st.dataframe(pd.DataFrame(rows_semis), use_container_width=True, hide_index=True)

        st.caption(
            f"Previsões geradas em **{semis_preds_raw.get('_gerado_em','')[:10]}** · "
